...
pub fn default_provider() -> Provider {
    ...
    #[cfg(all(feature = "aws-lc-rs", not(feature = "ring"), not(feature = "pure")))]
    {
        return Arc::new(rustls::crypto::aws_lc_rs::default_provider());
    }
    #[cfg(all(feature = "ring", not(feature = "aws-lc-rs"), not(feature = "pure")))]
    {
        return Arc::new(rustls::crypto::ring::default_provider());
    }
    #[cfg(feature = "pure")]
    {
        // Process default was already checked above — nothing more we can do without
        // depending on a concrete provider crate from this crate.
        panic!(
            "CryptoProvider::install_default() must be called before WebTransport (call moq_native_ietf::install_pure_crypto() or install rustls-rustcrypto)"
        );
    }
