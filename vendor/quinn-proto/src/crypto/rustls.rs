...
impl crypto::Session for TlsSession {
    ...
    fn is_valid_retry(&self, orig_dst_cid: &ConnectionId, header: &[u8], payload: &[u8]) -> bool {
        ...
        {
            let (aad, tag) = pseudo_packet.split_at_mut(tag_start);
            super::pure_like::aes128_gcm_open_tag(&key, &nonce, aad, tag)
        }
    }
...
impl crypto::ServerConfig for QuicServerConfig {
    ...
    fn retry_tag(&self, version: u32, orig_dst_cid: &ConnectionId, packet: &[u8]) -> [u8; 16] {
        ...
        #[cfg(all(feature = "rustls-pure", not(feature = "ring"), not(feature = "aws-lc-rs")))]
        {
            super::pure_like::aes128_gcm_seal_tag(&key, &nonce, &pseudo_packet)
        }
    }
}
...
pub(crate) fn configured_provider() -> Arc<rustls::crypto::CryptoProvider> {
    ...
    #[cfg(all(feature = "rustls-aws-lc-rs", not(feature = "rustls-ring"), not(feature = "rustls-pure")))]
    {
    ...
    #[cfg(all(feature = "rustls-pure", not(feature = "rustls-ring")))]
    {
        panic!(
            "no rustls CryptoProvider installed; call rustls_rustcrypto::provider() install_default() before using Quinn"
        );
    }
    #[allow(unreachable_code)]
    {
        unreachable!("no crypto backend feature")
    }
}
